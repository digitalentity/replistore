...
> **Status: FIXED** — `ensureConnected`/`connectLocked` now take the caller's context and thread it through the TCP dial (`DialContext`), the SMB negotiate/session-setup (`DialConn`), and the tree connect (`Session.WithContext(ctx).Mount`), so a reconnect is bounded by the operation's own deadline instead of the lifecycle context or the 5 s dial timeout. The resulting session and share are rebound to `context.Background` so the long-lived connection — and later `Umount`/`Logoff` and per-call I/O — does not inherit a caller deadline that has since expired. The `go.kvsh.ch/smb2` migration already made every SMB call context-aware (per-call `File.WithContext`/`Share.WithContext`), so the watchdog the original fix proposed is unnecessary; it and the now-unused lifecycle `b.ctx` field were removed. Covered by `TestSMB_ReconnectHonorsContextDeadline`.
